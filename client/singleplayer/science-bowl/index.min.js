...
const h=document.getElementById("start");h&&h.addEventListener("click",()=>{try{hideAIHelpSection()}catch(a){}}),a.message(USER_ID,{type:"start"}),console.log("About to initialize AI help...");// Initialize immediately if DOM is ready, otherwise wait
...
const a=document.getElementById("question"),b=document.getElementById("answer-display");if(!a||!b)return void console.error("Question or answer elements not found");const c=a.textContent.trim(),d=b.textContent.trim(),e=d.replace(/^ANSWER:\s*/i,""),f=!!(window.room?.tossup?.is_mcq&&Array.isArray(window.room?.tossup?.options)),g=f?window.room.tossup.options:void 0;// Strip the leading "ANSWER:" label if present before sending to API
...
const h=await fetch("/api/ai-help/explain",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:c,answer:e,category:getCurrentCategory(),isMcq:f,options:g})});if(!h.ok){const a=await h.json();throw new Error(a.error||"Failed to get AI explanation")}const i=h.headers.get("content-type")||"";if(!i.includes("application/json")){const a=await h.text();throw new Error("Unexpected non-JSON response: "+a.slice(0,60))}const j=await h.json();// Hide loading and display explanation
hideLoadingState(),displayExplanation(j.explanation)}catch(a){console.error("Error getting AI explanation:",a),hideLoadingState();// Show error message
